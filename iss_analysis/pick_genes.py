...
def compute_means(exons_df, classify_by, gene_filter='\d', zeisel=False):
    if zeisel:
        exons_matrix = exons_df.filter(regex=gene_filter).to_numpy()[:,-27998:]
        expression_by_cluster = exons_df.groupby([classify_by]).mean().filter(regex=gene_filter).iloc[:,-27998:]
...
def train_test_split(exons_df, classify_by, gene_filter, efficiency=0.01, zeisel=False):
    train = {}
    test = {}
